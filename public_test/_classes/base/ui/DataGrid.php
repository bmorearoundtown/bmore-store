...
class DataGrid {
	...
	public function draw() {
		...
		$html = '
			<table class="table table-bordered table-striped table-condensed table-hover' . ($this->sortable ? ' sortable' : '') . ($this->clickable ? ' hoverable' : '') . '" id="' . $this->gridName . 'Grid">';
		...
		if (!$this->data || !count($this->data)) {
			...
			foreach ($this->data as $id => $vals) {
				...
				if ($this->alternateRows && (!array_key_exists('_alt', $vals) || $vals['_alt'])) {
					$alternate = !$alternate;
				}
			
				$html .= '
				<tr class="' . ($this->colorRows ? ($alternate ? 'roweven' : 'rowodd') : '') . ($vals['css'] ? ' ' . $vals['css'] : '') . ($this->clickable || $vals['clickable'] ? ' clickable' : '') . '" ' . ($vals['hide'] ? ' style="display: none;"' : '') . ' id="row' . $id . '">';
			...
			if ($this->alternateRows) $alternate = !$alternate;

			while (++$rowCount <= $this->minRows) {

				$html .= '
				<tr class="' . ($alternate ? 'roweven' : 'rowodd') . ' nohover">';
