...
//		this.options.currentFormat = this.options.timePicker ? this.options.dateTimeFormat : this.options.dateFormat;
		...
		if (this.options.icon)
			this.element.setStyle({ background: '#fff url(' + this.options.icon + ') 98% center no-repeat' });
	...
	datePicked: function(date) {
		...
		if (this.options.onDateSelect) this.options.onDateSelect(this.element);
	},
...
//			cell = row.insertCell(1);
			...
			cell.colSpan = 4;
			cell.innerHTML =this.tr('Exact minutes:');
			...
			otherInput.className = 'uiText';
			otherInput.type = 'text';
			otherInput.maxLength = 2;
			otherInput.style.width = '4em';
...
//				cell = row.insertCell(2);
			...
			if (this.options.datePicker)
				selectButton.value = this.tr('Set Date and Time');
			else
				selectButton.value = this.tr('Set Time');
