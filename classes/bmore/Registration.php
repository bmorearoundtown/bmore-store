...
class Registration extends DatabaseObject {
	...
	public function loadUniqueEmails() {
		$this->query("
			SELECT		*
			FROM		{$this->tableName}
	...
	public function getTotalPaidRegistrationCount($intEventId) {
		...
		$objQuery->query("
			...
			FROM		{$this->tableName}
			WHERE		eventId = {$intEventId} AND
						(
							(isActive = 1 AND ipnResponse LIKE '%[payment_status] => Completed%') OR
							(isActive IS NULL AND datePaid IS NULL)
						)
		");
		
		$intCount = $objQuery->getField('registrationCount');
		
		return $intCount ? $intCount : 0;
	...
	public function getTotalPaidTicketsCount($intEventId) {
		...
		$intCount = $objQuery->getField('ticketCount');
	...
	public function sendDevelopmentConfirmationEmail() {
		...
		mail( 'scottymitch88@gmail.com', 'Bmorearoundtown.com Confirmation Email ( DEVELOPMENT )', $strHtml , $headers );
